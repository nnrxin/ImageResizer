...
Class ADO
{
	...
	static Open(filePath, HDR := true, readOnly := false, password := "")
	{
		try
		{
			...
			conn.Open(ADO.ConnectionString(filePath, HDR, readOnly, password, &fileType))
		}
		catch
			return
		return ADO(filePath, fileType, conn, readOnly)
	}
	...
	static Read(filePath, HDR := false, password := "")
	{
		...
		{
			conn := ComObject("ADODB.Connection")
			conn.Open(ADO.ConnectionString(filePath, HDR, true, password, &fileType))
		}
		catch
			return
		;获取表信息
	...
	static Write(db, filePath, HDR := false, password := "")
	{
		...
		{
			conn := ComObject("ADODB.Connection")
			conn.Open(ADO.ConnectionString(filePath, HDR, false, password, &fileType))
		}
		catch
			return
		;向各表添加信息
