...
/*
	...
	*/
	static Enum(flags := "")
	{
		...
		if !(DllCall("winspool.drv\EnumPrinters", "uint", flags, "ptr", 0, "uint", 2, "ptr", 0, "uint", 0, "uint*", &size := 0, "uint*", 0))
		{
			buf := Buffer(size << 1, 0)
			if (DllCall("winspool.drv\EnumPrinters", "uint", flags, "ptr", 0, "uint", 2, "ptr", buf, "uint", buf.Size, "uint*", 0, "uint*", &enum := 0))
			{
				addr := buf.Ptr, PRINTER_INFO := Map()
				loop enum
				{
					PRINTER_INFO[A_Index] := {}
					try PRINTER_INFO[A_Index].ServerName  := StrGet(NumGet(addr + 0, "ptr"))
					try PRINTER_INFO[A_Index].PrinterName := StrGet(NumGet(addr + A_PtrSize, "ptr"))
					try PRINTER_INFO[A_Index].ShareName   := StrGet(NumGet(addr + A_PtrSize * 2, "ptr"))
					try PRINTER_INFO[A_Index].PortName    := StrGet(NumGet(addr + A_PtrSize * 3, "ptr"))
					try PRINTER_INFO[A_Index].DriverName  := StrGet(NumGet(addr + A_PtrSize * 4, "ptr"))
					try PRINTER_INFO[A_Index].Comment     := StrGet(NumGet(addr + A_PtrSize * 5, "ptr"))
					try PRINTER_INFO[A_Index].Location    := StrGet(NumGet(addr + A_PtrSize * 6, "ptr"))
	...
	static GetDefault()
	{
		...
		{
			size := VarSetStrCapacity(&buf, size << 1)
