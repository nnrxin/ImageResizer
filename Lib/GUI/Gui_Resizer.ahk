...
;   Class Gui_ReSizer
;
;   Call: Gui_ReSizer(GuiObj, WindowMinMax, Width, Height)
;
;   Parameters:
;	1) {GuiObj} 		Gui Object
...
;
;   	Normally parameters are passed by a callback from {gui}.OnEvent("Size", Gui_ReSizer)
;
...
;
;	Gui Properties:
;		Gui_ReSizer_Init		{Gui}.Gui_ReSizer_Init := 1, will cause all controls of the Gui to be redrawn on next function call
;                               {Gui}.Gui_ReSizer_Init := 2, will also reinitialize abbreviations
;
Class Gui_ReSizer
{
    ;{ Call Gui_ReSizer
    Static Call(GuiObj, WindowMinMax, GuiW, GuiH)
    {
        ...
        Try
            (GuiObj.Gui_ReSizer_Init)
        Catch
            GuiObj.Gui_ReSizer_Init := 2 ; Redraw Twice on Initial Call(called on initial Show)
        ...
        For Hwnd, CtrlObj in GuiObj
        {
            ...
            ;}
            ;{ Redraw on Cleanup or GuiObj.Gui_ReSizer_Init
            If GuiObj.Gui_ReSizer_Init or (CtrlObj.HasProp("Cleanup") and CtrlObj.Cleanup = true)
                CtrlObj.Redraw()
            ;}
        ...
        ;}
        ;{ Reduce GuiObj.Gui_ReSizer_Init Counter and Check for Call again
        If (GuiObj.Gui_ReSizer_Init := Max(GuiObj.Gui_ReSizer_Init - 1, 0))
        {
            GuiObj.GetClientPos(, , &AnchorW, &AnchorH)
            Gui_ReSizer(GuiObj, WindowMinMax, AnchorW, AnchorH)
        }
        ;}
        ;{ Functions: Helpers
    ...
    ;{ Methods:
    ;{ Options
    Static Opt(CtrlObj, Options) => Gui_ReSizer.Options(CtrlObj, Options)
    ...
    ;}
    ;{ Now
    Static Now(GuiObj, Redraw := true, Gui_ReSizer_Init := 2)
    {
        If Redraw
            GuiObj.Gui_ReSizer_Init := Gui_ReSizer_Init
        GuiObj.GetClientPos(, , &Width, &Height)
        Gui_ReSizer(GuiObj, WindowMinMax := 1, Width, Height)
    }
    ;}
    ;}
}
